...
/**
 * @details     Start the A/D conversion
 */
#define _ADC_START_CONVERT()                (ADC->ADCR |= ADC_ADCR_ADST_Msk)

/**
 * @details     Stop the A/D conversion
 */
#define _ADC_STOP_CONVERT()                 (ADC->ADCR &= ~ADC_ADCR_ADST_Msk)

/**
 * @details     Enable the A/D converter
 */
#define _ADC_CONVERTER_ENABLE()             (ADC->ADCR |= ADC_ADCR_ADEN_Msk)

/**
 * @details     Disable the A/D converter
 */
...
/**
 * @details     Select the STADC pin as the external trigger source
 */
#define _ADC_SELECT_EXT_TRG_PIN()           (ADC->ADCR &= (~ADC_ADCR_TRGS_Msk))

/**
 * @details     Select the PWM as the external trigger source.
 */
#define _ADC_SELECT_EXT_TRG_PWM()           (ADC->ADCR = ADC->ADCR & (~ADC_ADCR_TRGS_Msk) | ADC_ADCR_TRGS_PWM)
...
/**
 * @details     Check if the A/D conversion is done or not
 */
#define _ADC_IS_CONVERSION_DONE()            ((ADC->ADSR & ADC_ADSR_ADF_Msk) >> ADC_ADSR_ADF_Pos)

/**
 * @details     A while-loop to check if the A/D conversion is done or not. Exit the while-loop if ADC conversion is done
 */
#define _ADC_WAIT_CONVERSION_DONE()          while(!_ADC_IS_CONVERSION_DONE())
...
/**
 * @brief       Comparator 0 configuration
 *
 ...
 * @return      None
 *
 * @details     Set comparator 0 parameters including compare channel, condition, compare data and match count.
 */
...
/**
 * @brief       Comparator 1 configuration
 *
 * @param[in]   CmpCh      Compare ADC Channel , should be:
 *                          -  0-7
 * @param[in]   CmpCond    Compare Condition , should be:
 ...
 * @return      None
 *
 * @details     Set comparator 1 parameters including compare channel, condition, compare data and match count.
 */
 ...
 * @return      None
 *
 * @details     The function is used to enable ADC channels.
 *
 */
...
/**
 * @details    Get the number of the channel which is in converting or will be converted.
 */
#define _ADC_GET_CURRENT_CONVERSION_CHANNEL()    ((ADC->ADSR & ADC_ADSR_CHANNEL_Msk) >> ADC_ADSR_CHANNEL_Pos)
 ...
 *             It only needs 21 ADC clocks to complete an A/D conversion when the ADC clock source is HCLK.
 */
