...
{
    return ((((UART_GetClockFreq()/UART_GetClockDivider() * 2) / u32BaudRate + 1) >> 1) - 2);
}
...
extern __inline void UART_SetRTS_TrgLev(UART_T* UART,uint32_t eActLevel,uint32_t eTRG_Level)
{
 ...
 *
 */ 
 extern __inline void UART_ResetIP(UART_T* UART)
{
...
extern __inline int32_t UART_Write(UART_T *UART,uint8_t *pu8TxBuf,uint32_t u32WriteBytes)
{
...
extern __inline int32_t UART_Read(UART_T *UART,uint8_t *pu8RxBuf, uint32_t u32ReadBytes)
{
    uint32_t u32Count, u32delayno;
